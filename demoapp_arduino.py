...
from braillecontroller import BrailleHardwareController
...
class FlashcardApp:
    ...
    def __init__(self, root):
        ...
        self.hardware = BrailleHardwareController()
        self.use_hardware = False
        self.button_rating_pending = False  # Flag for hardware rating
        
        # Setup UI additions
        self.setup_ui()
        self.setup_hardware_ui()
        
        # Check for Arduino button presses periodically
        self.check_hardware_buttons()
        self.check_hardware_show_hide()
        
        self.load_due_cards()
        self.next_card()

    def check_hardware_show_hide(self):
        """Periodically check for show/hide button presses from Arduino"""
        if self.hardware.connected:
            show_hide = self.hardware.get_show_hide()
            if show_hide is not None:
                self.toggle_answer(from_hardware=True)  # Pass True for hardware

        # Check again in 100ms (ADD THIS LINE!)
        self.root.after(100, self.check_hardware_show_hide)

    def setup_hardware_ui(self):
        """Add hardware control to UI"""
        # Create a NEW frame at the TOP of the window (not in left panel)
        hw_frame = tk.Frame(self.root, bg="#34495e", height=40)
        hw_frame.pack(fill=tk.X, side=tk.TOP, before=self.root.winfo_children()[0])  # Put at very top
        hw_frame.pack_propagate(False)
        
        # Hardware status (left side)
        status_frame = tk.Frame(hw_frame, bg="#34495e")
        status_frame.pack(side=tk.LEFT, padx=10)
        
        self.hw_status = tk.Label(
            status_frame,
        ...
        self.hw_status.pack(side=tk.LEFT, padx=5)
        
        # Connection button (middle)
        ...
        # Hardware toggle (right side)
        toggle_frame = tk.Frame(hw_frame, bg="#34495e")
        toggle_frame.pack(side=tk.RIGHT, padx=10)
        
        self.hw_var = tk.BooleanVar(value=self.use_hardware)
        hw_check = tk.Checkbutton(
            toggle_frame,
            text="Use Braille Hardware",
            variable=self.hw_var,
            command=self.toggle_hardware_display,
            bg="#34495e",
            fg="white",
            selectcolor="#34495e",
        ...
        hw_check.pack(side=tk.LEFT, padx=5)
        
        # Hardware rating indicator
        self.hw_rating_indicator = tk.Label(
            toggle_frame,
            text="Press Arduino buttons 1-4 to rate",
            font=("Arial", 9),
            bg="#34495e",
            fg="#ecf0f1"
        )
        self.hw_rating_indicator.pack(side=tk.LEFT, padx=10)

        # Add a label to show button functions
        button_info = tk.Label(
            hw_frame,
            text="Arduino Buttons: 1-4=Rate, 5=Show/Hide",
            font=("Arial", 8),
            bg="#34495e",
            fg="#bdc3c7"
        )
        button_info.pack(side=tk.LEFT, padx=10)
    ...
    def toggle_connection(self):
        ...
        if self.hardware.connected:
            ...
            self.connect_btn.config(text="🔌 Connect Arduino", bg="#3498db")
            self.hw_status.config(text="❌ Disconnected", fg="#e74c3c")
            self.hw_var.set(False)
            self.use_hardware = False
        else:
            if self.hardware.connect():
                self.connect_btn.config(text="🔌 Disconnect", bg="#e74c3c")
                self.hw_status.config(text="✅ Connected", fg="#2ecc71")
                messagebox.showinfo(
                    "Connected!",
                    "Arduino connected successfully!\n\n"
                    "Use the checkbox to enable the physical Braille display.\n"
                    "Press buttons 1-4 on Arduino to rate cards."
                )
                # If hardware was previously enabled, restore display
                if self.hw_var.get() and self.current_card:
                    self.hardware.send_character(self.current_card)
            else:
                ...
                if not ports:
                    ...
                    port_descriptions = [f"{port.device} - {port.description}" 
                                        for port in ports]
                    
                    # Create selection dialog
                    port_window = tk.Toplevel(self.root)
                    port_window.title("Select Arduino Port")
                    port_window.geometry("400x200")
                    ...
                    port_var = tk.StringVar(value=port_names[0])
                    ...
                    port_menu.pack(pady=10, padx=20)
                    
                    def connect_selected():
                        selected = port_var.get().split(' - ')[0]
                        if self.hardware.connect(selected):
                            port_window.destroy()
                            self.connect_btn.config(text="🔌 Disconnect", bg="#e74c3c")
                            self.hw_status.config(text="✅ Connected", fg="#2ecc71")
                        else:
    ...
    def toggle_hardware_display(self):
        """Enable/disable hardware display"""
        self.use_hardware = self.hw_var.get()
        if self.use_hardware:
            ...
            # DISABLING HARDWARE - Clear the display
            if self.hardware.connected:
                self.hardware.clear_display()
    
    def update_hardware_display(self, character):
        """Update physical Braille display"""
        if self.use_hardware and self.hardware.connected:
            self.hardware.send_character(character.lower())
    
    def check_hardware_buttons(self):
        """Periodically check for button presses from Arduino"""
        if self.hardware.connected and self.use_hardware:
            rating = self.hardware.get_rating()
            if rating is not None:
                self.process_hardware_rating(rating)
        
        # Check again in 100ms
        self.root.after(100, self.check_hardware_buttons)
    
    def process_hardware_rating(self, rating):
        """Process rating from Arduino hardware buttons"""
        if not self.current_card or not self.show_answer:
            # Not ready for rating yet
            self.hw_rating_indicator.config(
                text="Show answer first!",
                fg="#e74c3c"
            )
            self.root.after(2000, lambda: 
                self.hw_rating_indicator.config(
                    text="Press 1-4 on Arduino",
                    fg="#95a5a6"
                ))
            return
        
        # Map Arduino rating to color
        rating_colors = {
            0: "#e74c3c",  # Again (Red)
            3: "#e67e22",  # Hard (Orange)
            4: "#2ecc71",  # Good (Green)
            5: "#3498db"   # Perfect (Blue)
        }
        
        # Visual feedback
        self.hw_rating_indicator.config(
            text=f"Rating: {rating}",
            fg=rating_colors.get(rating, "#95a5a6")
        )
        
        # Rate the card automatically
        self.rate_card(rating, True)
    ...
    def setup_ui(self):
        ...
        self.left_panel = tk.Frame(main_container, bg="white", relief=tk.RAISED, borderwidth=2)
        self.left_panel.pack(side=tk.LEFT, fill=tk.Y, padx=(0, 10))
        
        # Algorithm Selector
        algo_frame = tk.Frame(self.left_panel, bg="white")
        ...
        # Separator
        ttk.Separator(self.left_panel, orient='horizontal').pack(fill=tk.X, padx=20, pady=10)
        ...
        for label, key in stats_info:
            frame = tk.Frame(self.left_panel, bg="white")
        ...
        self.show_answer_btn = tk.Button(
            ...
            text="👁️ Show Answer",
            font=("Arial", 14, "bold"),
            command=lambda: self.toggle_answer(from_hardware=False),  # Pass False
            bg="#2ecc71",
            fg="white",
            relief=tk.FLAT,
            padx=30,
    ...
    def next_card(self):
        ...
        if self.use_hardware and self.current_card:
            self.update_hardware_display(self.current_card)
    ...
    def toggle_answer(self, from_hardware=False):
        """Toggle between question and answer.
        
        Args:
            from_hardware: True if called from Arduino button, False if from app button
        """
        if not self.show_answer:
            ...
            # Enable hardware rating
            self.button_rating_pending = True
            self.hw_rating_indicator.config(
                text="Ready for rating (Press 1-4)",
                fg="#2ecc71"
            )
        else:
            self.show_answer = False
            ...
            # Only disable hardware rating if NOT from hardware button
            # (When hardware button hides, rating should stay enabled for next press)
            if not from_hardware:
                self.button_rating_pending = False
                self.hw_rating_indicator.config(
                    text="Press Show/Hide button",
                    fg="#95a5a6"
                )
            else:
                # Keep rating pending for next hardware press
                self.hw_rating_indicator.config(
                    text="Ready - Press rating button",
                    fg="#2ecc71"
                )
    
    def rate_card(self, rating: int, from_hardware=False):
        ...
        # Only show popup for software ratings, not hardware
        if not from_hardware:
    ...
    def reset_all(self):
        ...
        if confirm:
            # Clear hardware display if connected and enabled
            if self.hardware.connected and self.use_hardware:
                self.hardware.clear_display()
...
def main():
    ...
    # Handle window close
    def on_closing():
        if app.hardware.connected:
            app.hardware.clear_display()
            app.hardware.disconnect()
        root.destroy()
